...
public class AccountApiHelper extends D3BankingApi {
    ...
    public D3BankingApi excludeAccount(String accountName) throws D3ApiException {
        logger.info("Attempting to exclude the account named: {}", accountName);
        ...
        logger.info("Found Account to edit");
        ...
        logger.info("Account: {} updated successfully (excluded)", accountName);
        return this;
    }

    public D3BankingApi hideAccount(String accountName) throws D3ApiException {
        logger.info("Attempting to hide the account named: {}", accountName);
        List<Account> accounts = getAccounts();
        if (accounts == null || accounts.isEmpty()) {
        ...
        logger.info("Account: {} updated successfully (hidden)", accountName);
        return this;
    }

    public D3BankingApi enrollInEstatements(D3User user, Integer accountId) throws D3ApiException, IOException {
        logger.info("Attempting to enroll in E-Statements for user: {} using accountId {}", user, accountId);
        ...
        logger.info("EStatement enrollment was successful for user: {}", user);
        return this;
    }

    public D3BankingApi addOfflineAccount(D3Account offlineAccount) throws D3ApiException {
        logger.info("Attempting to add offline account");
        BankingService service = retrofit.create(BankingService.class);
        ...
        try {
            ...
            logger.error("Error adding offline account of type {}", offlineAccount.getProductType().toString());
            throw new D3ApiException("Error adding offline account");
        }
        logger.info("Offline account: {} was added successfully", offlineAccount.getName());
        return this;
    }
    ...
    public D3BankingApi addOfflineAccountTransaction(D3User user, D3Transaction transaction, D3Account offlineAccount, String category) throws D3ApiException {
        logger.info("Attempting to add offline transaction for {}'s account {} ", user.getLogin(), offlineAccount.getName());
        BankingService service = retrofit.create(BankingService.class);
        ...
        logger.info("Adding offline transaction for {}'s account {} was successful", user.getLogin(), offlineAccount.getName());
        return this;
    }
