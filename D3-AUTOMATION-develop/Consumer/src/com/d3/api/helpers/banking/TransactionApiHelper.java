...
public class TransactionApiHelper extends D3BankingApi {
    ...
    public void addCategory(D3User user, String... categoryType) throws D3ApiException {
        ...
        logger.info("Attempting to add {} category for user: {} ", type, user);
        ...
        logger.info("Adding {} category {} was successful for user: {}", type, category.getName(), user.getLogin());
    }

    public void addCategoryRule(int transactionId, int categoryId) throws D3ApiException {
        logger.info("Attempting to add category rule with id {} to transaction with id {} ", categoryId, transactionId);
        BankingService service = retrofit.create(BankingService.class);
        ...
        logger.info("Adding category rule {} was successful", categoryId);
    }

    public void addRenamingRule(int transactionId) throws D3ApiException {
        logger.info("Attempting to add renaming rule to transaction with id {} ", transactionId);
        BankingService service = retrofit.create(BankingService.class);
        try {
            Transaction transactionInfo = service.getTransactionInfo(transactionId).execute().body();
        ...
        logger.info("Adding renaming rule to transaction with id {} was successful", transactionId);
    }
}
