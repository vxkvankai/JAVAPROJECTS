...
public class ConduitBuilder {
    ...
    public String saveConduitFileLocally() {
        ...
        logger.info("Attempting to save file: {}", temp.getPath());
        ...
        try (FileWriter fileWriter = new FileWriter(temp)) {
            ...
            logger.error("There was a problem trying to save the conduit file {}", e);
            return null;
        }
    }
     ...
     */
    public ConduitBuilder uploadFileToApi(String fi, String baseApiUrl, String apiVersion) throws ConduitException, D3ApiException {
        logger.info("Logging into access conduit API");
        try {
            ConduitApiHelper api = new ConduitApiHelper(baseApiUrl, apiVersion);
            api.login(DatabaseUtils.getRandomUserName(fi), "password", "denver");
            logger.info("Sending Conduit file through API");
            api.sendConduitFileToApi(getDocument());
            return this;
        } catch (D3ApiException e) {
            logger.error("Conduit API call failed", e);
            throw e;
        }
    }
    ...
    private ConduitBuilder addCompany(D3Company company) throws ConduitException {
    ...
    private ConduitBuilder addUser(D3User user) {
        ...
        addSingleAttribute("uid", user.getUid())
                ...
                .addSingleAttribute("del", String.valueOf(user.isDelete()))
                .addSingleElement("login", user.getLogin(), true)
                .addSingleElement("ut", user.getLevelValue(), true)
                .addSingleElement("enrl", String.valueOf(user.getEnrolledValue()))
                .addUserProfile(user)
                .addUserAccountList(user.getAccountPairs())
