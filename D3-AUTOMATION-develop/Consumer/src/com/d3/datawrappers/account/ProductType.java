...
public enum ProductType {
    ASSET(AccountingClass.ASSET, "Asset Account"), //For Offline Asset Accounts. Can't be added through conduit
    ...
    public AccountingClass getAccountingClass() {return accountingClass;}

    public Integer getAccountProductId() {
        try (DatabaseUtils utils = new DatabaseUtils()) {
            String query =
                    "SELECT id FROM account_product WHERE source_product_id = ''{0}''";
            return Integer.parseInt(utils.getDataFromSelectQuery("id", query, this.toString()));
        } catch (SQLException | D3DatabaseException e) {
            LoggerFactory.getLogger(ProductType.class).error("Error getting account product id for {} \n{}", this, e);
            return null;
        }
    }
