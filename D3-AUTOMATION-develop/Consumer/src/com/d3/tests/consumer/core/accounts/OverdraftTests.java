...
public class OverdraftTests extends AccountsTestBase {
    ...
    private MyAccountsSection overdraftCommonSteps(D3User user, D3Account account) {
        DatabaseUtils.updateCompanyAttributeValueString("accounts.overdraft.protection.enabled", "true");
        DatabaseUtils.updateCompanyAttributeValueString("overdraft.protection.pdf.verification.enabled", "true");
        DatabaseUtils.updateCompanyAttributeValueString("accounts.overdraft.protection.mobile.enabled", "true");
        DatabaseUtils.updateCompanyAttributeValueString("accounts.overdraft.protection.checks.enabled", "true");
        DatabaseUtils.updateAccountProductPermission(account.getProductType(), "d3Permission.overdraft.protection", "true");
    ...
    public void verifyAccountOptIn(D3User user) throws D3DatabaseException {
        ...
        Assert.assertTrue(DatabaseUtils.areOverdraftOptionsCorrect(accountToUse, OverdraftStatus.OPT_IN, OverdraftStatus.OPT_IN),
        ...
        Assert.assertTrue(DatabaseUtils.areOverdraftOptionsCorrect(accountToUse, OverdraftStatus.REVOKE, OverdraftStatus.REVOKE),
            "Overdraft db status not as expected");
