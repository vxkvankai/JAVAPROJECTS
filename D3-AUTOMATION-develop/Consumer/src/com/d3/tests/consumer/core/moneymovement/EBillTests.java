...
public class EBillTests extends MoneyMovementTestBase {
    ...
    public void verifyStopReceivingPaperBills(D3User user, Recipient recipient) {
        ...
        Assert.assertNotNull(DatabaseUtils.getAuditRecordId(user, Audits.PAPER_SUPPRESSION_SUBMITTED));
    ...
    public void verifyEbillFile(D3User user, Recipient recipient) {
        ...
        Assert.assertNotNull(DatabaseUtils.getAuditRecordId(user, Audits.EBILL_FILED));
    ...
    public void verifyEBillPayNow(D3User user, Recipient recipient) {
        ...
        payNow.clickContinueButton().clickOkButton();

        Schedule schedulePage = dashboard.getHeader()
            .clickMoneyMovementButton().getTabs().clickScheduleLink()
        ...
        Assert.assertNotNull(DatabaseUtils.getAuditRecordId(user, Audits.PAYMENT_SUBMITTED), "PAYMENT_SUBMITTED Audit record not found for user");
    ...
    public void verifyAutoPaySearchResults(D3User user, Recipient recipient) {
        ...
        EBillsPage eBillsPage = dashboard.getHeader().clickMoneyMovementButton().getTabs().clickEbillsLink()
            .enterSearchTerm(RandomHelper.getRandomString(10))
            .clickSearchButton();
        Assert.assertTrue(eBillsPage.isTextDisplayed(EBillsL10N.Localization.NO_EBILLS_RECEIVED.getValue()), "No Ebill Receive L10N not displayed");
        ...
        eBillsPage.clickClearDateRangeButton()
            ...
            .clickSearchButton();
        Assert.assertTrue(eBillsPage.filteredEbillsContainText(recipient.getName()), "Received Ebill does not contain the filtered text");
        ...
        eBillsPage.clickDateRangeButton()
            ...
            .clickDateRangeSearchButton();
        Assert.assertTrue(eBillsPage.filteredEbillsContainText(transferDetails.getTransfer().getAmountStr()), "Received Ebill does not contain the filtered text");
