...
public class OutOfBandRecipientsTests extends MoneyMovementTestBase {
    ...
    public void verifyOOBCodeRequiredForAddingRecipient(D3User user, Recipient recipient) {
        ...
        if (recipient instanceof CompanyBillPaySeedRecipient) {
            logger.warn("This Test (Company Bill Pay Seed Recipient) will not work as expected due to REL31-225");
            return;
        }
        ...
        Assert.assertTrue(oobPage.isTextDisplayed(CommonL10N.Localization.OOB_VERIFICATION_SENT.getValue()));

        String verificationCode = DatabaseUtils.getOOBVerificationCode(user);
