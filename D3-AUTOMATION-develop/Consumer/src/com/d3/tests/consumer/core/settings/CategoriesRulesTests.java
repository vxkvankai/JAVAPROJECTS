...
public class CategoriesRulesTests extends SettingsTestBase {
    ...
    public void verifyRenameCategory(D3User user) {
        ...
        categories.clickCategory("TEST CATEGORY").changeCategoryName(newCategoryName);

        TransactionsPage transactions = dashboard.getHeader().clickTransactionsButton();
        transactions.clickCategoryName();
        Assert.assertTrue(transactions.doesCategoryExist(newCategoryName), String.format("Category Name %s was not displayed", newCategoryName));
    }
