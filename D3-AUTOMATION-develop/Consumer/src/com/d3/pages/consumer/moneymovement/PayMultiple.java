...
public class PayMultiple extends MoneyMovementBasePage {
    ...
    public boolean isCorrectTemplateSelected(String template) {
        logger.info("Checking if Pay Multiple dropdown is set to {}", template);
        try {
            checkIfTextEquals(getCurrentTemplateTitle(), template);
        } catch (TextNotDisplayedException e) {
            logger.warn("Correct template was not selected", e);
            return false;
        }
    ...
    public boolean isPaymentTotalCorrect(String paymentAmount) {
        logger.info("Checking if Pay Multiple Payment Total is equal to {}", paymentAmount);
        try {
            ...
            if (value.isPresent()) {
                ...
                logger.warn("Value/element was not present");
                return false;
            }
        } catch (TextNotDisplayedException e) {
            logger.warn("Payment total was not correct", e);
            return false;
        }
    ...
    public boolean isDeleteWorksheetConfirmTextDisplayed() {
        try {
            ...
            logger.warn("Delete Worksheet Confirm message not displayed", e);
            return false;
        }
    ...
    public boolean areSubmittedWorksheetConfirmationDetailsCorrect(List<PayMultipleTransfer> transfers) {
        ...
        String expectedPaymentTotal = String.format("$%,.2f", transfers.stream().collect(Collectors.summingDouble(D3Transfer::getAmount)));
        ...
        try {
            ...
            logger.error("Worksheet summary did not display label or Payment total correctly", e);
            return false;
        }
        return true;
    }
    ...
    private boolean isIndividualWorksheetItemInfoCorrect(PayMultipleTransfer transfer) {
        ...
        try {
            ...
            logger.error("Individual worksheet item did not contain the correct information", e);
            return false;
        }
        return true;
    }
