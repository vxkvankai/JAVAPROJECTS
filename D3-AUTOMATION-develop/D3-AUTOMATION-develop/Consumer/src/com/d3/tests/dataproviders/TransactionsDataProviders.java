...
public class TransactionsDataProviders extends DataProviderBase {
    ...
    public Object[][] getBasicUserWithAnImageTransaction(Method method) {
        ...
        try {
            ...
            log.error("Issue creating a user with lots of transactions", e);
        }

        if (user == null) {
            log.error("Issue getting user");
        ...
        for (int i=0; i<30; ++i) {
            if (transaction != null && !transaction.isPending()) {
                log.info("Using transaction: {} for testing", transaction);
                break;
            } else {
                log.info("Issue with transaction: {}, getting new one", transaction);
            }
            transaction = user.getRandomTransactionFromChecking();
        }

        if (transaction == null) {
            log.error("Couldn't get a good transaction");
            return new Object[][] {{null, null}};
        }
