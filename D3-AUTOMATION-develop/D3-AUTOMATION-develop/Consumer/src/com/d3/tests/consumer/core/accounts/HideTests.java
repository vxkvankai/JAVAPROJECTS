...
public class HideTests extends AccountsTestBase {
    ...
    public void verifyHideAccount(D3User user, D3Account account) {
        ...
        log.info("Checking to see if {} is still on the screen", account.getName());
    ...
    @TmsLink("522302")
    @TmsLink("522303")
    @Story("Hide Account Not Available For Account With Certain alerts added")
    @RunForSpecificAlerts(d3AlertType = {AccountBalanceAlert.class, TransactionAmountExceedsAlert.class, CreditDepositAlert.class})
    ...
    public void validateHiddenAccountsValuesAreNotIncludedInBudgetCalculations(D3User user, D3Account account) {
        ...
        String month = TransactionDatabaseHelper.getPostDateDebitTransactionForBudget(account);
        String sumDebitTransaction = TransactionDatabaseHelper.getSumDebitTransactionForBudget(account);
        ...
        String sumOfUsersDebitTransactions = TransactionDatabaseHelper.getSumOfUsersDebitTransactionsInSpecifyMonth(user, month);
    ...
    public void verifyDashboardManageAndPlanTotalsAreUpdatedWhenAllAccountsAreHidden(D3User user) {
        ...
        for (BigDecimal actual : newCurrentMonthInfo) {
            Assert.assertTrue(DateAndCurrencyHelper.compareCurrency(actual, BigDecimal.ZERO),
                String.format("Value on the dashboard: %s, was not zero", actual));
        }

        //Get Net Worth
        List<BigDecimal> newNetWorthInfo = dashboard.clickPlanButton().getNetWorthInfo();

        for (BigDecimal actual : newNetWorthInfo) {
            Assert.assertTrue(DateAndCurrencyHelper.compareCurrency(actual, BigDecimal.ZERO),
                String.format("Value on the Network dashboard: %s, was not zero", actual));
        }
    }
    ...
    public void verifyMessageIsDisplayedWhenAllAccountsAreHidden(D3User user) {
        ...
        List<String> accountsAfterHide = AccountDatabaseHelper.getMaskedUserAccounts(user);

        SingleTransferForm payTransfer = dashboard.clickPayTransferButton();
        Assert.assertTrue(payTransfer.areDestinationAccountsCorrect(accountsAfterHide));
    ...
    public void verifyHiddenAccountsAreNotDisplayedOnTransactions(D3User user, D3Account account) {
        ...
        Assert.assertTrue(transactions.areCorrectTransactionsDisplayed(AccountDatabaseHelper.getMaskedUserAccounts(user)));
    }
    ...
    public void verifyHiddenAccountsAreNotDisplayedOnMoneyMovement(D3User user, D3Account account) {
        ...
        Assert.assertTrue(singleTransfer.areDestinationAccountsCorrect(AccountDatabaseHelper.getMaskedUserAccounts(user)));
        Assert.assertTrue(singleTransfer.areSourceAccountsCorrect(AccountDatabaseHelper.getMaskedUserAccounts(user)));
    ...
    public void verifyHiddenAccountsAreNotDisplayedInNetWorthCalculationsAndTotalsAreUpdated(D3User user) {
        ...
        assertThat(dashboardPlanPage).hasCorrectNetWorthCalculations(user)
            .netWorthTotalsHaveChanged(currentNetWorthInfo, newCurrentNetWorthInfo);
    }
    ...
    public void verifyHiddenAccountsAreNotDisplayedOnDashboardManageAndTotalsAreUpdated(D3User user, D3Account account) {
        ...
        SingleTransferForm payTransfer = dashboard.getHeader()
            .clickDashboardButton()
            .clickPayTransferButton();
        Assert.assertTrue(payTransfer.areDestinationAccountsCorrect(accountsAfterHide));
        payTransfer.clickCancelButton();

        assertThat(dashboard).hasCorrectRecentTransactionAccounts(accountsAfterHide)
            .doesNotDisplayText(accountName)
