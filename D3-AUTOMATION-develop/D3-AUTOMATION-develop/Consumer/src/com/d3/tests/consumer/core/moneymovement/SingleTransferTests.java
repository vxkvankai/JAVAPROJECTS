...
public class SingleTransferTests extends MoneyMovementTestBase {
    ...
    public void validateFailedStatusForInternalTransfers(D3User user) {
        ...
        String auditTransactionStatus = AuditDatabaseHelper.getAuditRecordAttributes(user, Audits.TRANSFER_PROCESSED)
    ...
    public void verifySuccessfulCurrentDateTransferWithAuditRecord(D3User user, SingleTransfer transfer) {
        ...
        String auditTransactionStatus = AuditDatabaseHelper.getAuditRecordAttributes(user, transfer.getProcessedTransferAuditType())
            .getOrDefault(AuditAttribute.STATUS_CODE.getAttributeName(), "");
        Assert.assertFalse(auditTransactionStatus.isEmpty());
    ...
    public void verifyDashboardTransferIsScheduledNextDayIfCutoffTimeIsPassed(D3User user) {
        ...
        payTransferPage.enterScheduledDate(transfer.getScheduledDate());
    ...
    @TmsLink("308942")
    @Story("Single Transfer cutoff time")
    @Test(dataProvider = "User with not submitted Bill Pay transfer for current date")
    public void verifyDashboardBillPayTransferCannotScheduleForCurrentDate(D3User user, D3Transfer transfer) {
        SingleTransferForm singleTransferForm = login(user)
            .clickPayTransferButton()
            .fillOutForm(transfer)
            .clickSubmitButton();

        Assert.assertTrue(singleTransferForm.isTextDisplayed(ScheduleL10N.Localization.ENTER_SCHEDULED_DATE_ERROR.getValue()),
            "Expected error message did not show");
    }


    @TmsLink("307408")
    @Story("Single Principal Only Payment")
    ...
    @Test(dataProvider = "Basic User")
    public void verifySinglePrincipalOnlyPaymentCurrentDate(D3User user) {
        SingleTransfer transfer = SingleTransfer.createRandomTransfer(user.getFirstAccountByType(ProductType.CREDIT_CARD), RandomHelper.getRandomElementFromList(user.getAssetAccounts()));
        ...
        Map<String, String> auditAttributes = AuditDatabaseHelper.getAuditRecordAttributes(user, Audits.TRANSFER_PROCESSED);
        Assert.assertNotNull(auditAttributes);
        Assert.assertEquals(auditAttributes.getOrDefault(AuditAttribute.STATUS_CODE.getAttributeName(), ""), D3Transaction.TransactionStatus.PROCESSED.toString());
    ...
    @RunWithAccountProductAttribute(accountProduct = ProductType.CREDIT_CARD, attribute = AccountProductAttributes.PRINCIPAL_ONLY_PAYMENT)
    @Test(dataProvider = "Basic User")
    public void verifySinglePrincipalOnlyPaymentFutureDate(D3User user) {
        SingleTransfer transfer = SingleTransfer.createRandomTransfer(user.getFirstAccountByType(ProductType.CREDIT_CARD), RandomHelper.getRandomElementFromList(user.getAssetAccounts()));
        SingleTransferForm form = login(user).getHeader()
            .clickMoneyMovementButton()
            .clickScheduleButton();
        ...
        Map<String, String> auditAttributes = AuditDatabaseHelper.getAuditRecordAttributes(user, Audits.TRANSFER_SCHEDULED);
        Assert.assertNotNull(auditAttributes);
        Assert.assertEquals(auditAttributes.getOrDefault(AuditAttribute.STATUS_CODE.getAttributeName(), ""), D3Transaction.TransactionStatus.PENDING.toString());
        Assert.assertEquals(auditAttributes.getOrDefault(AuditAttribute.PRINCIPAL_ONLY.getAttributeName(), ""), String.valueOf(true));
    }

    @AfterClass(alwaysRun = true)
    public void resetCutoffTime() {
        AttributeDatabaseHelper.updateCompanyAttributeValueString(CompanyAttribute.MONEY_MOVEMENT_INTERNAL_TRANSFER_CUTOFF, "11:59 PM");
    }
}
