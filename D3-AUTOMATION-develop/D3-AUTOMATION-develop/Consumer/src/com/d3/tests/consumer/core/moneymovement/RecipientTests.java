...
public class RecipientTests extends MoneyMovementTestBase {
    ...
    public void verifyConsumerDeletingRecipient(D3User user, Recipient recipient) {
        ...
        Map<String, String> events = AuditDatabaseHelper.getAuditRecordAttributes(user, Audits.RECIPIENT_DELETED);

        Assert.assertFalse(events == null || events.isEmpty(), "The audit event was not created");
        Assert.assertTrue(events.get(AuditAttribute.RECIPIENT_NAME.getAttributeName()).equals(recipient.getName()), // NOSONAR, assert above
    ...
    public void verifyAddingCheckFreeRecipientPersonSpecialChars(D3User user, List<Recipient> recipients) {
        ...
        Assert.assertTrue(form.isTextDisplayed(L10nCommon.Localization.CHARACTERS_NOT_ALLOWED.getValue()));
    ...
    public void verifyEditingRecipient(D3User user, Recipient existingRecipient) {
        ...
        Map<String, String> audit = AuditDatabaseHelper.getAuditRecordAttributes(user, Audits.RECIPIENT_UPDATED);
    ...
    @TmsLink("309009")
    @Story("Add company recipient is successful when selecting Manual Entry")
    @Test(dataProvider = "Basic User With Bill Pay")
    public void verifyAddCompanyRecipientManualEntry(D3User user) {

        CompanyBillPayManualRecipient recipient = CompanyBillPayManualRecipient.createRandomRecipient(false);
        ...
        RecipientsPage recipientsPage = login(user).getHeader()
            ...
            .clickRecipientsLink()
            .clickAddRecipientButton()
            .clickCompanyToPayButton()
            .clickBillAddressInfoButton(recipient)
            .enterRecipientInfo(recipient)
            .clickSaveButton();

        Assert.assertTrue(recipientsPage.isTextDisplayed(recipient.getName()), "New Recipient name is not listed in the recipient list");

        SingleTransferForm singleTransferForm = recipientsPage.getHeader().clickDashboardButton().clickPayTransferButton();

        Assert.assertTrue(singleTransferForm.verifyNewRecipientToDropDown(recipient.getNickname()), "New Recipient name is not listed in the Pay transfer To Account");


        Assert.assertNotNull(AuditDatabaseHelper.getAuditRecordId(user, Audits.RECIPIENT_CREATED));
