...
public class TransactionToggleTests extends TransactionsTestBase {
    ...
    public void verifyAvailableAccountsToUser(D3User user) {
        ...
        Assert.assertTrue(transactions.areAvailableAccountsCorrect(AccountDatabaseHelper.getMaskedUserAccounts(user)));
    }
    ...
    public void verifyConsumerCategoriesDisplayForConsumerAccounts(D3User user) {
        String accountName = AccountDatabaseHelper.getRandomAccountWithPostedTransaction(user, D3UserProfile.ProfileType.CONSUMER);
        ...
        Dashboard dashboard = login(user);
        SingleTransactionPage transactions = dashboard.getHeader()
            .clickTransactionsButton()
            .filterByAccounts(account)
    ...
    public void verifyBusinessCategoriesAreAvailableForBusinessAccounts(D3User user) {
        String accountName = AccountDatabaseHelper.getRandomAccountWithPostedTransaction(user, D3UserProfile.ProfileType.BUSINESS);
        List<D3Account> account = user.getAccounts()
            .stream()
            .filter(accounts -> accounts.getName().equals(accountName)).collect(Collectors.toList());

        Dashboard dashboard = login(user);
        SingleTransactionPage transactions = dashboard
            .getHeader()
            .clickTransactionsButton()
            .filterByAccounts(account)
            .clickFirstPostedTransaction()
