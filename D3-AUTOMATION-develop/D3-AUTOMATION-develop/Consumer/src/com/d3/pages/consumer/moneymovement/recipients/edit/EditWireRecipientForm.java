...
public class EditWireRecipientForm extends PageObjectBase implements EditRecipientsForm<EditWireRecipientForm, WireRecipient> {
    ...
    public boolean isFormInformationCorrect(WireRecipient wireRecipient) {
        try {
            log.info("Checking if routing number: {} is on the form", wireRecipient.getRoutingNumber());
            ...
            log.info("Checking if financial institution: {} is on the form", wireRecipient.getFinancialInstitution());
            checkIfTextEquals(getFinancialInstitution(), wireRecipient.getFinancialInstitution());

            log.info("Checking if address 1: {} is on the form", wireRecipient.getAddress1());
            checkIfTextEquals(getAddress1(), wireRecipient.getAddress1());

            log.info("Checking if address 2: {} is on the form", wireRecipient.getAddress2());
            checkIfTextEquals(getAddress2(), wireRecipient.getAddress2());

            log.info("Checking if city: {} is on the form", wireRecipient.getCity());
            checkIfTextEquals(getCity(), wireRecipient.getCity());

            log.info("Checking if state: {} is on the form", wireRecipient.getState());
            checkIfTextEquals(getState(), wireRecipient.getState());

            log.info("Checking if zip: {} is on the form", wireRecipient.getZipCode());
            checkIfTextEquals(getZipCode(), wireRecipient.getZipCode());
        } catch (TextNotDisplayedException e) {
            log.warn("Wire Recipient form was not validated", e);
            return false;
        }
