...
public class OtherAccountsSection extends Accounts<OtherAccountsSection> {
    ...
    @Step("Check if the online account format is displayed")
    public boolean isOnlineAccountformationDisplayed() {

        String paymentText = paymentDueLabel.getText();
        if (paymentText.isEmpty()) {
            log.warn("Online account labels are empty: {}", paymentText);
            return false;
        }

        String lastSuccessText = lastSuccessfulLabel.getText();
        if (lastSuccessText.isEmpty()) {
            log.warn("Online account labels are empty: {}", lastSuccessText);
            return false;
        }
        return true;
    }
    ...
    public boolean areOnlineAccountLabelsDisplayed() {
        ...
        try {
            ...
            log.warn("Online account field check failed:", e);
            return false;
        }
