...
public class MoneyMovementApiHelper extends D3BankingApi {
    ...
    public D3BankingApi enrollInBillPay(D3User user, int accountId) throws D3ApiException {
        log.info("Attempting to enroll in bill pay for user: {} with accountId of {}", user, accountId);
        ...
        log.info("Bill pay enrollment was successful for user: {}", user);
        return this;
    }

    public D3BankingApi addRecipient(ApiRecipient recipient) throws D3ApiException {
        log.info("Attempting to add recipient: {}", recipient);
        try {
        ...
        log.info("Recipient adding was successful, {}", recipient);
        return this;
    }

    public D3BankingApi addRecurringTransfer(ApiRecurringTransfer transfer) throws D3ApiException {
        log.info("Attempting to add transfer: {}", transfer);
        try {
        ...
        log.info("Transfer adding was successful, {}", transfer);
        return this;
    }
    ...
    public MoneyMovementApiHelper addRecipientsToUser(D3User user, List<Recipient> recipients, boolean setAllAsActive)
        ...
        for (Recipient recipient : recipients) {
            log.info("Creating recipient: {}", recipient);
    ...
    public SearchResults getSearchResultsForBillPay(String searchTerm) throws D3ApiException {
        log.info("Attempting to get search results for bill pay provider, {}", searchTerm);
        BankingService service = retrofit.create(BankingService.class);
    ...
    public D3BankingApi addPayMultipleTemplate(D3User user, List<PayMultipleTransfer> payments) throws D3ApiException {
        log.info("Attempting to add pay multiple worksheet template alert for user {}", user.getLogin());
        ...
        log.info("Creating Worksheet Template was successful for user: {}", user);
        return this;
    }
    ...
    public D3BankingApi activateEbiller(D3User user, Recipient recipient) throws D3ApiException {
        log.info("Attempting active E-Biller for user {}", user.getLogin());
        Integer providerId = RecipientMMDatabaseHelper.getExternalEndpointProviderId(user, recipient.getName());
        BankingService service = retrofit.create(BankingService.class);
        ...
        log.info("Activating E-Biller {} for user: {} was successful", recipient.getName(), user);
        return this;
    }
    ...
    public D3BankingApi addAutoPayToEbiller(D3User user, Recipient recipient) throws D3ApiException {
        log.info("Attempting to add Auto Pay to E-Biller for user {}", user.getLogin());
        Integer providerId = RecipientMMDatabaseHelper.getExternalEndpointProviderId(user, recipient.getName());
        ...
        log.info("Adding AutoPay for E-Biller {} for user: {} was successful", recipient.getName(), user);
        return this;
    }
